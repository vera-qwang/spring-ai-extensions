 ...
 *
 * <p>
 * This class wraps the Bailian SDK and provides reactive API methods for knowledge base
 * operations. It handles client initialization, request construction, and error handling.
 *
 * <p>
...
public class BailianClient {
	...
	public BailianClient(BailianConfig config) throws Exception {
		...
		log.info("BailianClient initialized for workspace: {} at endpoint: {}", workspaceId, config.getEndpoint());
	}
	 ...
	 *
	 * <p>
	 * This constructor allows injecting a mock SDK client for unit testing without
	 * requiring actual Alibaba Cloud credentials.
	 ...
	 *
	 * <p>
	 * This method searches the specified knowledge base (index) for documents relevant to
	 * the given query. Results are returned sorted by relevance score, limited to the
	 ...
	 * <p>
	 * This method searches the specified knowledge base (index) for documents relevant to
	 * the given query. If conversation history is provided and query rewrite is enabled
	 * in the configuration, the query will be rewritten based on the conversation
	 * context.
	 *
	 * <p>
	 * Retrieval parameters (denseSimilarityTopK, sparseSimilarityTopK, reranking, etc.)
	 * are read from the BailianConfig provided during client construction.
	 * @param indexId the knowledge base ID to search in
	 * @param query the search query text
	 * @param limit the maximum number of documents to retrieve (null for default)
	 * @param conversationHistory the conversation history for multi-turn rewrite (can be
	 * null)
	 * @return a Mono emitting the retrieve response
	 */
	public Mono<RetrieveResponse> retrieve(String indexId, String query, Integer limit,
		...
		return Mono.fromCallable(() -> {
			log.debug("Retrieving from index: {} with query: {} (limit: {}, historySize: {})", indexId, query, limit,
			...
			if (config.getRerankConfig() != null) {
				...
				RetrieveRequest.RetrieveRequestRerank rerank = new RetrieveRequest.RetrieveRequestRerank();
			...
			if (config.getRewriteConfig() != null) {
				...
				List<RetrieveRequest.RetrieveRequestRewrite> rewriteList = new ArrayList<>();
				RetrieveRequest.RetrieveRequestRewrite rewrite = new RetrieveRequest.RetrieveRequestRewrite();
			...
			if (conversationHistory != null && !conversationHistory.isEmpty()) {
				List<RetrieveRequest.RetrieveRequestQueryHistory> historyList = new ArrayList<>();
				for (QueryHistoryEntry entry : conversationHistory) {
					RetrieveRequest.RetrieveRequestQueryHistory historyEntry = new RetrieveRequest.RetrieveRequestQueryHistory();
			...
			RetrieveResponse response = sdkClient.retrieveWithOptions(workspaceId, request, new HashMap<>(), runtime);
			...
			if (response.getBody().getData() == null) {
				log.warn("Bailian API returned no data for query: {}. requestId={},statu={}," + " code={}, message={}",
						query, response.getBody().getRequestId(), response.getBody().getStatus(),
						response.getBody().getCode(), response.getBody().getMessage());
			}

			log.debug("Retrieved {} documents from index: {}",
					response.getBody().getData() != null && response.getBody().getData().getNodes() != null
							? response.getBody().getData().getNodes().size() : 0,
					indexId);
	 ...
	 * @throws Exception if client creation fails due to invalid credentials (invalid
	 * access key ID or secret), network connectivity issues, SDK initialization errors,
	 * or invalid endpoint configuration
	 */
