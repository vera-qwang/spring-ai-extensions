...
public final class DashScopeAgent extends Agent {
  ...
  private DashScopeAgentRequest toRequest(Prompt prompt, Boolean stream) {
    ...
    boolean onlyOneUserMessage =
        messages.size() == 1 && messages.get(0).getMessageType() == MessageType.USER;
    if (onlyOneUserMessage) {
      ...
      requestMessages =
          messages.stream()
              .map(
                  msg ->
                      new DashScopeAgentRequestMessage(
                          msg.getMessageType().getValue(), msg.getText()))
              .toList();
    }
    ...
    return new DashScopeAgentRequest(
        ...
        new DashScopeAgentRequest.DashScopeAgentRequestParameters(
            ...
            ragOptions == null
                ? null
                : new DashScopeAgentRequestRagOptions(
                    ragOptions.getPipelineIds(),
                    ragOptions.getFileIds(),
                    ragOptions.getMetadataFilter(),
                    ragOptions.getTags(),
                    ragOptions.getStructuredFilter(),
                    ragOptions.getSessionFileIds())));
  }
